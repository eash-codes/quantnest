...
class Portfolio:
    def __init__(self, wallet_id: str, market: MarketProvider):
        ...
        self._market = market  # ← FIXED: _market not market
        self._positions: Dict[str, Decimal] = {}
        self._trades: List[Trade] = []

    @property
    def wallet(self) -> 'Wallet':
        """Public access to wallet (read-only)."""
        return self._wallet
    ...
    def buy(self, symbol: str, quantity: Decimal) -> None:
        ...
        price = self._market.get_price(symbol)  # ← FIXED: _market
        cost = price * quantity
    ...
    def sell(self, symbol: str, quantity: Decimal) -> None:
        ...
        trade = Trade(symbol, "SELL", quantity, price)  # ← FIXED: price not proceeds
        self._trades.append(trade)
